...
`notes/` 是资料消化层，用来把视频、文章、书、课程、会议、聊天记录、概念解释和工具观察转成自己的中文笔记。

这里不是收藏夹。每篇笔记至少要完成一件事：把来源材料转成自己的理解、问题清单、使用边界或工程判断。

## 与其他目录的边界

- 只保存来源入口，不写理解：放 `../sources/`。
- 单篇或小来源簇的资料消化：放 `notes/`。
- 跨来源、稳定、可复用的主题知识：升级到 `../knowledge/`。
- 围绕一个问题形成结构化判断：升级到 `../research/`。
- 需要跑代码验证判断：升级到 `../labs/`。
- 产出 PPT、报告、文章或方案：放 `../outputs/`，再把新判断回写到 `../knowledge/` 或 `../research/`。
...
- 升级到 `knowledge/`：已经跨来源整理，能作为长期主题知识复用。
- 升级到 `research/`：已经围绕明确问题形成判断、证据和边界。
- 升级到 `labs/`：需要验证一个具体 API、SDK、工具链、失败模式或工程判断。

旧文件不强制补元数据。复查或升级时再使用 [note 模板](../methods/templates/note.md)。
