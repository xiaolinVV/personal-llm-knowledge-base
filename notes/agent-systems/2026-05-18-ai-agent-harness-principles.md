...
- 后续应进入：暂不升级；若要验证，可进入 `labs/` 做一个最小 Harness 实验，验证 JSON schema 校验、工具参数校验、输出清洗、SVG 安全扫描和视觉检查闭环。
...
- 是否现在就做：否。当前任务是视频资料消化；实验应该单独开 `labs/agent-harness-svg-demo/`，并明确验收标准。
