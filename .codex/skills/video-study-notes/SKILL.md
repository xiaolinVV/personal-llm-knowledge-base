...
4. Write the fieldbook note.
   ...
   - Default to `20-资料笔记/` as the first landing zone for video-derived material.
   ...
   - If the video mainly supports deeper technical research or case-study work, state the suggested next home under `40-问题研究/`.
   - Only recommend upgrading to `50-实验验证/` when the video contains a concrete API, SDK, toolchain, failure mode, or engineering claim worth validating with a minimal runnable experiment.
...
- A structured fieldbook note under `20-资料笔记/<original-video-title>.md`.
