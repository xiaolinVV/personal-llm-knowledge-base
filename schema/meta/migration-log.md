...
## 2026-06-02：全量迁移为 Karpathy raw/wiki/schema 模式

### 核心判断

旧的生命周期顶层目录已经能表达“来源、笔记、知识、研究、实验、输出”的区别，但它还不是 Karpathy gist 里的 LLM Wiki 模式。真正缺的是三层边界：原始证据、LLM 编译后的 wiki、以及 agent 维护规约。

因此本次迁移删除旧生命周期顶层目录，不保留跳转壳，改成：
...
| 原路径 | 新路径 | 理由 |
| --- | --- | --- |
| `inbox/` | `raw/inbox/` | 待处理线索属于证据层。 |
| `sources/` | `raw/sources/` | 来源卡片、网页快照和原始材料属于证据层。 |
| `notes/` | `wiki/notes/` | 已消化内容属于 LLM 编译层。 |
| `knowledge/` | `wiki/topics/` | 稳定知识改名为主题页，贴近 wiki 语义。 |
| `research/` | `wiki/research/` | 问题驱动研究属于 wiki 编译层。 |
| `labs/` | `wiki/labs/` | 最小验证实验属于 wiki 编译层。 |
| `outputs/` | `wiki/outputs/` | 成品和 manifest 属于 wiki 输出层。 |
| `archive/` | `wiki/archive/` | 历史材料仍归入 wiki。 |
| `methods/templates/` | `schema/templates/` | 模板属于规约层。 |
| `methods/` | `schema/methods/` | 可复用方法属于规约层。 |
| `meta/` | `schema/meta/` | 仓库治理属于规约层。 |
| `meta/obsidian/` | `schema/obsidian/` | Obsidian 视图属于规约层。 |

### 特殊处理

- `notes/ragflow/official-practice-cases/` 下的网页快照、图片、HTML、metadata、dataset 和文章镜像迁入 `raw/sources/ai/ragflow/official-practice-cases/`。这些是证据，不是资料消化笔记。
- `docs/README.md` 删除。它只是旧入口兼容壳，本次选择不保留旧顶层目录。
- `.codex/skills/` 保留在根目录。它是 Codex 可执行 Skill 配置，不按内容层迁移。
- 根目录 `AGENTS.md` 保留作为 Codex 入口，并指向 `schema/agent-protocol.md`。

### 新增文件

- `wiki/index.md`：内容索引。
- `wiki/log.md`：append-only 时间线日志。
- `schema/agent-protocol.md`：ingest、query、lint、output write-back 维护协议。

### 未验证事项

- 本次验证只覆盖路径结构、Git rename、忽略规则和本地 Markdown 链接解析。
