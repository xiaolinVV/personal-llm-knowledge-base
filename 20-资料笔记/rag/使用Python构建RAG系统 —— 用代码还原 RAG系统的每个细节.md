...
- 后续应进入：优先补到 `50-实验验证/openai/03-tools-and-rag/` 的对照实验；如果要保留 Gemini 版本，再单独建一个最小 RAG lab。
...
- 可放置位置：优先扩展 `50-实验验证/openai/03-tools-and-rag/`，把生成模型换成当前主线 OpenAI 技术栈；如果要严格复现视频，再建 Gemini 版本对照。
...
- 本地概念笔记：[RAG 工作机制详解——一个高质量知识库背后的技术全流程](RAG%20工作机制详解——一个高质量知识库背后的技术全流程.md)
