...
- 后续应进入：如要研究企业落地，应进入 `40-问题研究/use-cases/` 做场景拆解；只有在明确要验证 RAGFlow 的解析质量、召回质量、权限隔离或 API 集成时，才升级到 `50-实验验证/`。
...
- 是否现在就做：否。当前任务是视频沉淀；实验需要单独建 `50-实验验证/`，并明确模型、embedding、数据集和验收标准。
