...
```bash
uv run python labs/openai/02-agents-sdk-basic/support_agent_cli.py ORDER-1001 --local-tools-only
```

运行完整 Agents SDK 实验：

```bash
uv run python labs/openai/02-agents-sdk-basic/support_agent_cli.py \
...
uv run python labs/openai/02-agents-sdk-basic/support_agent_cli.py \
  ...
  --session-db labs/openai/02-agents-sdk-basic/.sessions/lab02.sqlite
```
