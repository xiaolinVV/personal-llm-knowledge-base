...
- 后续应进入：如要研究 RAGFlow 的检索质量，应另开 `wiki/labs/` 最小实验，对同一数据集比较 chunking、embedding、rerank、引用准确率和答案可追溯性；如要做工具调研，可进入 `wiki/research/open-source-projects/` 拆 RAGFlow 架构。
